...
#$storeClass = Get-SCOMClass -Name "SystemCenterCentral.Utilities.Certificates.CertificateStore"
$storeClass = $scomMG.GetMonitoringClasses('SystemCenterCentral.Utilities.Certificates.CertificateStore')[0]

#$hsObject = Get-SCOMClassInstance -Class $hsClass | where {$_.DisplayName -eq $wfOriginatorComputerName}
...
#$storeObject = Get-SCOMClassInstance -Class $storeClass | where {($_.Name -eq $wfOriginatorStoreName) -and ($_.Path -eq $wfOriginatorComputerName)}
...
#$discoveries = @(Get-SCOMDiscovery -Name "SystemCenterCentral.Utilities.Certificates.LocalScriptProbe.NonRootCertificate.Discovery";
#	Get-SCOMDiscovery -Name "SystemCenterCentral.Utilities.Certificates.LocalScriptProbe.RootCertificate.Discovery";
#	Get-SCOMDiscovery -Name "SystemCenterCentral.Utilities.Certificates.LocalScriptProbe.SelfSignedCertificate.Discovery";
#	Get-SCOMDiscovery -Name "SystemCenterCentral.Utilities.Certificates.LocalScriptProbe.CRL.Discovery")
...
if (($hsObject -ne $null) -and ($storeObject -ne $null)) {
	...
	foreach ($discovery in $discoveries) {
		...
		#$discoveryOverrideResults = $storeObject.GetResultantOverrides($discovery) #| % {$_.ResultantPropertyOverrides | where {$_.Keys -eq 'Enabled'}}
		...
		foreach ($discoveryOverrideResult in $discoveryOverrideResults) {
			...
			#$discoveryEnabled = $discoveryOverrideResult.Values.EffectiveValue
		}
